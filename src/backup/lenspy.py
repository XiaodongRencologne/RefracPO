...
from .pocal import epsilon,mu
from .pocal import PO_GPU_2 as PO_GPU

from .vecops import  Vector as vector, norm as abs_v
from .Vopy import scalarproduct
...
class simple_Lens():
    ...
    def PO_analysis(self,N1,N2,source,k,
        ...
        E_in.to_coordsys(matrix = np.matmul(self.coord_sys.mat_g_l,source.coord_sys.mat_l_g))
        H_in.to_coordsys(matrix = np.matmul(self.coord_sys.mat_g_l,source.coord_sys.mat_l_g))
        self.f_E_in = E_in
        self.f_H_in = H_in
        del(f1_p,f1_p_n)
        """
    ...
    def source(self,
        ...
        if isinstance(target,Spherical_grd) or isinstance(target,plane_grd):
            ...
            face2_n.y = data[1,:]
            face2_n.z = data[2,:]
            H2.to_coordsys(matrix = np.matmul(target.coord_sys.mat_g_l,self.coord_sys.mat_l_g))
            E2.to_coordsys(matrix = np.matmul(target.coord_sys.mat_g_l,self.coord_sys.mat_l_g))
