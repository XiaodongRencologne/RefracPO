...
def Creat_Fresnel_coeffi_AR(theta_i,t_p,r_p,t_s,r_s,n1,n2):
    ...
    def Fresnel_coeffi_AR1(theta):
        t_p = tp_AR(theta) + 1j * tp_AR_imag(theta) 
        t_s = ts_AR(theta) + 1j * ts_AR_imag(theta)
        r_p = rp_AR(theta) + 1j * rp_AR_imag(theta)
        r_s = rs_AR(theta) + 1j * rs_AR_imag(theta)
        return t_p,t_s,r_p,r_s
    def Fresnel_coeffi_AR2(theta):
        ...
        factor[NN_t] = 0.0
        t_p = (tp_AR(theta) + 1j * tp_AR_imag(theta)) * factor
        t_s = (ts_AR(theta) + 1j * ts_AR_imag(theta)) * factor
        r_p = rp_AR(theta) + 1j * rp_AR_imag(theta)
        r_s = rs_AR(theta) + 1j * rs_AR_imag(theta)
...
def calculate_Field_T_R_AR(n1,n2,
    ...
    return E_t,E_r,H_t,H_r, poynting_i, n2/n1*theta_t_cos/theta_i_cos*(np.abs(t_p)**2+np.abs(t_s)**2)/2, (r_p**2+r_s**2)/2,NN